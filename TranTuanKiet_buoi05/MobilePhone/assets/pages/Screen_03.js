...
export default function Screen_01({ navigation, route }) {
  const selectedData = route.params?.selectedData || {
    color: "Chưa chọn màu",
    provide: "Chưa có nhà cung cấp",
    price: "Chưa có giá",
    uri: null,  // Thêm uri để xử lý
  };
  return (
    <SafeAreaView style={{ flex: 1, margin: 15 }}>
      <View style={{alignItems: 'center'}}>
      {selectedData.uri && (
        <Image source={selectedData.uri} resizeMode="contain" />
      )}
      </View>
      ...
      <View style={{flexDirection: 'row'}}>
        <Text style={{fontWeight: 'bold', fontSize: 20,  marginTop: 10, marginRight: 25}}>{selectedData.price}</Text>
        <Text style={{fontWeight: '100', fontSize: 17, marginTop: 10, textDecorationLine: 'line-through'}}>{selectedData.price}</Text>
      </View>
      <View style={{flexDirection: 'row'}}>
